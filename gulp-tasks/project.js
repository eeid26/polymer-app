...
function getHttpCacheMapFromSW(swFileContent) {
  ...
  if (startIndex !== -1) {
    ...
    if (startIndex !== -1) {
      ...
      if (endIndex !== -1) {
        let mapContent = swFileContent.substring(startIndex, endIndex + 1);
        httpCacheMapFileContent += "var HTTPCacheConfig = " + mapContent;
      }
    }
  }
  return httpCacheMapFileContent;
}
 ...
 */
function addHttpCache(swFileContent) {
  return new Promise((resolve, reject) => {
    let httpCachePath = path.join(bundledPath, global.config.httpCacheConfig || 'http-cache-config.js');
    fs_1.writeFile(httpCachePath, getHttpCacheMapFromSW(swFileContent), (err) => {
      if (err) {
        reject(err);
      }
      else {
...
function httpCache() {
  ...
  return addHttpCache(fs_1.readFileSync(serviceWorkerPath, 'utf-8'))
}
